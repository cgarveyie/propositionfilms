...
class Reorder {
	...
	public function __construct( $args = array() ) {
		...
		add_action('wp_ajax_nopriv_post_sort_new', array( $this, 'ajax_save_post_order'  ));
		add_action('wp_ajax_post_sort_new', array( $this, 'ajax_save_post_order'  ) );
	...
	public function ajax_save_post_order() {
		...
		//Get JSON data
		$custom_order	= json_decode( str_replace( "\\", '', $_POST['typeorder'] ) );
		$post_data 		= json_decode( str_replace( "\\", '', $_POST['postorder'] ) );
	...
	*/
	private function output_row( $the_post ) {
		...
		$image = wp_get_attachment_image_src( get_post_thumbnail_id(), 'tinyfeatured' );

		// get image size
		list($width, $height) = getimagesize($image[0]);

		// if thumbnail has been made, show it.
		...
		<li id="list_<?php the_id(); ?>" class="custom-order-<?php echo get_post_meta($post->ID, 'homepage_order', true); ?>"><div><?php if ($imageshow) { echo '<img src="'.$image[0].'" width="20" height="20" />'; } ?> <?php the_title(); ?></div></li>
		<?php
	} //end output_row
	...
	*/
	...
	public function sort_posts() {
		...
		<style type="text/css">
		#icon-reorder-posts {
			background:url(<?php echo esc_url( $this->icon ); ?>) no-repeat;
		}
		</style>
		<div class="wrap">

			<h2>
				<?php echo esc_attr( $this->heading ); ?>
				<img src="<?php echo admin_url( 'images/loading.gif' ); ?>" id="loading-animation" />
			</h2>

            <p><?php esc_html_e('Drag and drop to reorder your posts. Changes are automatically saved.', 'themewich'); ?></p>
			...
			if ( ! $posts ) {
				...
				esc_html_e( 'You have no posts to reorder. You need to add some posts first!', 'themewich' );
				echo '</p></div>';
			} else {
