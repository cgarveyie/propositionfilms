<?php
/**
 * Class and functions for theme updates
 *
 * @package WordPress
 * @since 1.0
 */
...
if ( ! function_exists( 'themewich_get_update_info' ) ) :
	function themewich_get_update_info() {
		...
		$username = isset( $tw_options['tf_username'] ) ? $tw_options['tf_username'] : false;
		$api      = isset( $tw_options['tf_username'] ) ? $tw_options['tf_api'] : false;
...
if ( ! class_exists( "Envato_Protected_API" ) ) {
	class Envato_Protected_API {
		 ...
		 *
		 * @internal param The $string buyer's Username
		 * @internal param The $string buyer's API Key can be accessed on the marketplaces via My Account -> My Settings -> API Key
		 *
		 * @access    public
		 * @since     1.0
		 */
...
if ( ! function_exists( 'tw_themeforest_themes_update' ) ) {
	function tw_themeforest_themes_update( $updates ) {
	...
	add_filter( "pre_set_site_transient_update_themes", "tw_themeforest_themes_update" );
}