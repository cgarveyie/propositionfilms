...
( function( $ ) {
    ...
    media.view.Settings.Gallery = media.view.Settings.Gallery.extend({
        render: function() {
			...
			var gallerySettings = this.$el.closest('.gallery-settings'),
                ...
                galColumns      = this.$el.find('select.columns').closest('label'),
                ...
                var $valueGal 		= $(this).val();
                    ...
                    switch($valueGal) {
						...
						case 'gallery':
							...
							break;
                        case 'slideshow':
                            galColumns.fadeOut(500);
                            break;
                        default:
                            hideSettings.add(widthSettings).add(galColumns).fadeIn(500);
                            hideSettings.val(originalCrop);
                            break;
    ...
    wp.media.view.Settings.AttachmentDisplay = _AttachmentDisplay.extend({
        ...
        render: function() {
            ...
            this.$el.find('select.alignment option[value="left"]').before('<option value="full">Full Width</option>');			
			
			 // Add error message for use
            this.$el.append('<div class="clear"></div><div class="error notwide" style="display:none"><p>Your original image is may not be large enough for this layout option. It should be at least 1500px wide. Substitute size is being used.</p></div>');
